...
def _ppi_resting_mean_ms(heart_rate: float) -> int:
    return int(round(60000 / heart_rate))


def _ppi_resting_std_ms(heart_rate: float, hrv_rmssd: float) -> int:
    age_adjusted = hrv_rmssd * 0.28
    hr_adjusted = age_adjusted - max(0.0, heart_rate - 70.0) * 0.08
    return int(round(max(8.0, min(28.0, hr_adjusted))))
...
def _sample_baseline(
    ...
    hrv_rmssd_morning = int(round(baseline["hrv_rmssd"]))
    return {
        "heart_rate": heart_rate,
        "respiratory_rate": 16,
        "ppi_resting_mean_ms": _ppi_resting_mean_ms(heart_rate),
        "ppi_resting_std_ms": _ppi_resting_std_ms(heart_rate, hrv_rmssd_morning),
        "stress_score": 34,
        "systolic_bp": int(round(baseline["systolic_bp"])),
        "diastolic_bp": int(round(baseline["diastolic_bp"])),
        "spo2": int(round(baseline["spo2"])),
        "hrv_rmssd_morning": hrv_rmssd_morning,
    }
...
def _sample_wearable_baseline(
    ...
    *,
    ...
    sampled.update(
        {
            ...
            "ppi_resting_mean_ms": float(baseline["ppi_resting_mean_ms"]),
            "ppi_resting_std_ms": float(baseline["ppi_resting_std_ms"]),
            "spo2": float(baseline["spo2"]),
            "hrv_rmssd_morning": float(baseline["hrv_rmssd_morning"]),
        }
    )
    ...
    return {
        ...
        "ppi_resting_mean_ms": int(round(sampled["ppi_resting_mean_ms"])),
        "ppi_resting_std_ms": int(round(sampled["ppi_resting_std_ms"])),
        ...
        "ppg_noise_level": round(float(sampled["ppg_noise_level"]), 4),
        "ppg_amplitude": round(float(sampled["ppg_amplitude"]), 3),
        "ecg_rhythm": "sinus_rhythm",
    }


def _group_baseline_signals(baseline: dict[str, int], wearable_baseline: dict[str, Any]) -> dict[str, Any]:
    return {
        "heart_rate": baseline["heart_rate"],
        "respiratory_rate": wearable_baseline["respiratory_rate"],
        "ppi_resting_mean_ms": baseline["ppi_resting_mean_ms"],
        "ppi_resting_std_ms": baseline["ppi_resting_std_ms"],
        "stress_score": wearable_baseline["stress_score"],
        "systolic_bp": baseline["systolic_bp"],
        "diastolic_bp": baseline["diastolic_bp"],
        "spo2": baseline["spo2"],
        "hrv_rmssd_morning": wearable_baseline["hrv_rmssd_morning"],
        "ecg_rhythm": wearable_baseline["ecg_rhythm"],
    }
...
def _generate_profile_from_group(
    ...
    return {
        ...
        "baseline_signals": _group_baseline_signals(baseline, wearable_baseline),
    }
