...
from simulator.core.models import format_utc_datetime, parse_utc_datetime
...
def _missing_field_name(stream_name: str, rng: random.Random) -> str:
    if stream_name == "wearable_continuous":
        return rng.choice(["heart_rate", "respiratory_rate"])
    if stream_name == "wearable_steps_event":
        return rng.choice(["steps_count", "steps_rate_per_min", "activity_type"])
    if stream_name == "wearable_stress":
        return rng.choice(["stress_score", "stress_level"])
    if stream_name == "wearable_ppi_batch":
        return "ppi_intervals_ms"
    if stream_name == "wearable_motion_batch":
        return "motion_points"
    if stream_name == "wearable_battery":
        return "battery_level"
    if stream_name == "wearable_bp_triggered":
        return rng.choice(["systolic_bp", "diastolic_bp"])
...
def _apply_fault(
    *,
    ...
    if fault_type == "invalid_ppi_intervals":
        faulty["ppi_intervals_ms"] = [-1] * len(message.get("ppi_intervals_ms", [1]))
        return [faulty], _fault_log_entry(stream_name, message, fault_type, "Set all ppi_intervals_ms to -1.")
    ...
    if fault_type == "invalid_blood_pressure":
        faulty["systolic_bp"] = 70
        faulty["diastolic_bp"] = 95
        return [faulty], _fault_log_entry(stream_name, message, fault_type, "Set systolic_bp <= diastolic_bp.")

    if fault_type == "invalid_battery_level":
        faulty["battery_level"] = 110
        return [faulty], _fault_log_entry(stream_name, message, fault_type, "Set battery_level to 110.")

    if fault_type == "invalid_motion_value":
        if faulty.get("motion_points"):
            faulty["motion_points"][0]["acc_magnitude"] = -99.0
        return [faulty], _fault_log_entry(stream_name, message, fault_type, "Set first acc_magnitude to -99.0.")
