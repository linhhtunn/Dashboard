...
def build_feature_message(raw: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "features": {
            ...
            "rr_interval_ms_current": raw["rr_interval_ms"],
            "respiratory_rate_current": raw["respiratory_rate"],
            "stress_score_current": raw["stress_score"],
            "acc_x_current": raw["acc_x"],
            "acc_y_current": raw["acc_y"],
            "acc_z_current": raw["acc_z"],
            "gyro_x_current": raw["gyro_x"],
            "gyro_y_current": raw["gyro_y"],
            "gyro_z_current": raw["gyro_z"],
        },
        "data_state": "VALID",
        "created_at": utc_now(),
    }
...
def parse_float_field(raw: dict[str, Any], field: str) -> tuple[float | None, str | None]:
    try:
        return float(raw[field]), None
    except (TypeError, ValueError):
        return None, f"{field} must be a float-compatible value"
...
def validate_wearable_continuous(raw: dict[str, Any]) -> tuple[bool, str | None, str | None]:
    ...
    rr_interval_ms, reason = parse_int_field(raw, "rr_interval_ms")
    if reason:
        return False, reason, "rr_interval_ms"
    ...
    motion_fields = ["acc_x", "acc_y", "acc_z", "gyro_x", "gyro_y", "gyro_z"]
    motion_values = {}
    for motion_field in motion_fields:
        value, reason = parse_float_field(raw, motion_field)
        if reason:
            return False, reason, motion_field
        motion_values[motion_field] = value
    ...
    if not 250 <= rr_interval_ms <= 2_000:
        return False, "rr_interval_ms out of mock valid range 250-2000", "rr_interval_ms"
    ...
    for acc_field in ["acc_x", "acc_y", "acc_z"]:
        if not -8 <= motion_values[acc_field] <= 8:
            return False, f"{acc_field} out of mock valid range -8 to 8 g", acc_field
    for gyro_field in ["gyro_x", "gyro_y", "gyro_z"]:
        if not -20 <= motion_values[gyro_field] <= 20:
            return False, f"{gyro_field} out of mock valid range -20 to 20 rad/s", gyro_field
