...
LEGACY_TRIGGERED_QUEUE_NAMES = (
    "q.team2.wearable_bp_triggered",
    "q.team2.wearable_spo2_triggered",
    "q.team2.wearable_ecg_triggered",
)
...
def _ensure_queue(
    ...
    *,
    ...
    try:
        ...
        if getattr(probe_channel, "is_open", False):
            probe_channel.close()
...
def _delete_legacy_queue_if_exists(connection: Any, queue_name: str) -> None:
    import pika

    cleanup_channel = connection.channel()
    try:
        cleanup_channel.queue_declare(queue=queue_name, passive=True)
    except pika.exceptions.ChannelClosedByBroker as exc:
        if exc.reply_code == 404:
            return
        raise
    ...
    try:
        ...
        print(f"Deleted legacy RabbitMQ queue: {queue_name}")
    finally:
        if getattr(cleanup_channel, "is_open", False):
...
def declare_topology(channel: Any, settings: RabbitMQSettings) -> None:
    ...
    for queue in settings.queues.values():
        if queue["name"] not in declared_queues:
            ...
            declared_queues.add(queue["name"])
            if reused:
    ...
    if os.getenv("RABBITMQ_CLEAN_LEGACY_TRIGGERED_QUEUES", "1").strip().lower() in {"1", "true", "yes", "y", "on"}:
        for queue_name in LEGACY_TRIGGERED_QUEUE_NAMES:
            if queue_name not in declared_queues:
                _delete_legacy_queue_if_exists(connection, queue_name)
