...
export function LoginForm() {
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-[color:var(--cs-surface)] px-4 py-10">
      <div className="dashboard-surface w-full max-w-md rounded-[1.2rem] p-6 sm:p-8">
        <div className="flex flex-col items-center text-center">
          <span className="flex h-12 w-12 items-center justify-center rounded-[0.9rem] bg-[linear-gradient(135deg,var(--cs-primary),var(--cs-teal))] text-white shadow-[0_12px_26px_rgba(13,71,161,0.22)]">
            <Activity className="h-6 w-6" strokeWidth={2} />
          </span>
          <h1 className="mt-4 text-[1.35rem] font-semibold text-[color:var(--cs-heading)]">
            {ui.auth.title}
          </h1>
        ...
        <form className="mt-6 space-y-4" onSubmit={(event) => void handleSubmit(event)}>
          ...
          <button
            type="submit"
            disabled={loading}
            className="flex h-11 w-full items-center justify-center gap-2 rounded-[0.8rem] bg-[color:var(--cs-primary)] text-[14px] font-semibold text-white disabled:opacity-60"
          >
        ...
        <p className="mt-5 text-center text-[12px] text-[color:var(--cs-text-soft)]">
          <Link href="/" className="font-semibold text-[color:var(--cs-primary)] hover:underline">
            {ui.auth.backHome}
          </Link>
        </p>
      </div>
