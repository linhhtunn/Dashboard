...
export function OAuthButtons({ nextPath = "/patients" }: OAuthButtonsProps) {
  ...
  if (!isSupabaseAuthConfigured() || !isGoogleOAuthEnabled()) return null;
  ...
  return (
    <div className="space-y-4">
      <button
        ...
        className={`${dashboardSecondaryBtn} w-full disabled:opacity-60`}
      >
        {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <GoogleIcon />}
        {ui.auth.oauthGoogle}
      </button>

      <AuthDivider label={ui.auth.oauthDivider} />
    </div>
  );
}
